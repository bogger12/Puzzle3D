Shader "Unlit/InvertedHull2"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                v2f o;
                float4 objectPos = (v.vertex + normalize(v.vertex) * _OutlineWidth)*(_NormalVSWorldSpace) + (v.vertex + float4(normalize(v.normal) * _OutlineWidth, 1)) * (1-_NormalVSWorldSpace);
